...
const MatchDetailsPage = () => {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`http://192.168.125.42:5000/api/matches/${matchId}`)
      .then(res => res.json())
  ...
  useEffect(() => {
    ...
    const fetchProfiles = async (players) => {
      return Promise.all(players.map(async (p) => {
        const res = await fetch(`http://192.168.125.42:5000/user/${p.email}`);
        return res.ok
          ? await res.json()
  ...
  const handleRemoveSelectedPlayers = async () => {
    for (const { team, email } of selectedPlayers) {
      await fetch(`http://192.168.125.42:5000/api/matches/${matchId}/remove-player`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ team: team === 'A' ? 'teamA' : 'teamB', email }),
      });
    }
    const updatedMatch = await fetch(`http://192.168.125.42:5000/api/matches/${matchId}`).then(r => r.json());
        ...
        {selectSquadMode && !squadConfirmed && (
          <Box sx={{ display: 'flex', justifyContent: 'center', mt: 3, gap: 2 }}>
            <Button
              ...
              startIcon={<SportsCricketIcon />}
              onClick={async () => {
                await fetch(`http://192.168.125.42:5000/api/matches/${matchId}/set-squad`, {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
                  body: JSON.stringify({
