...
const MyCricketPage = () => {
  ...
  useEffect(() => {
    if (email) {
      ...
      fetch(`http://192.168.125.42:5000/api/matches/created-by/${encodeURIComponent(email)}`)
        .then(res => res.json())
  ...
  useEffect(() => {
    if (activeNav === 1 && activeSection === 2) {
      ...
      axios.get(`http://192.168.125.42:5000/api/tournament?createdByEmail=${userEmail}`)
  ...
  useEffect(() => {
    if (activeNav === 0 && activeSection === 0 && email) {
      ...
      fetch(`http://192.168.125.42:5000/api/matches/my-matches/${encodeURIComponent(email)}`)
        .then(res => res.json())
  ...
  const handleDeleteTournament = async (id) => {
    ...
    try {
      await axios.delete(`http://192.168.125.42:5000/api/tournament/${id}`);
                      ...
                      return (
                        ...
                        >
                          ...
                          <Box sx={{ position: 'relative', width: '100%', height: 160, background: '#e3e3e3' }}>
                            <img
                              src={`http://192.168.125.42:5000/api/tournament/${t._id}/icon`}
                      ...
                      <IconButton
                        ...
                        onClick={e => {
                          ...
                          if (window.confirm('Are you sure you want to delete this match?')) {
                            fetch(`http://192.168.125.42:5000/api/matches/${match._id}`, {
                              method: 'DELETE',
                            })
                              .then(res => res.json())
                              .then(data => {
