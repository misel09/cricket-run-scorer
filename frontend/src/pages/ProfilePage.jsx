...
const ProfilePage = () => {
  ...
  const handleSave = async () => {
    try {
      const res = await fetch(`http://192.168.125.42:5000/user/${encodeURIComponent(user.email)}`, {
        method: 'PUT',
  ...
  const handleProfilePicChange = (e) => {
    ...
    if (file) {
      ...
      reader.onload = async (ev) => {
        ...
        // Send to backend
        await fetch(`http://192.168.125.42:5000/user/${encodeURIComponent(user.email)}/profile-pic`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
