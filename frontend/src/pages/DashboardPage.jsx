...
const DashboardPage = () => {
  ...
  const fetchNotifications = useCallback(() => {
    fetch(`http://localhost:5000/user/${encodeURIComponent(email)}/notifications`)
      .then(res => res.json())
      .then(data => {
  ...
  const handleOpenNotifications = () => {
    ...
    fetch(`http://localhost:5000/user/${encodeURIComponent(email)}/notifications/markAllRead`, {
      method: 'POST'
    })
      .then(() => {
  ...
  useEffect(() => {
    if (search.trim()) {
      if (searchType === 'players') {
        fetch(`http://localhost:5000/search/users?name=${encodeURIComponent(search.trim())}`)
          .then(res => res.json())
          .then(data => {
        ...
        fetch(`http://localhost:5000/search/tournaments?name=${encodeURIComponent(search.trim())}`)
          .then(res => res.json())
          .then(data => {
        ...
        fetch(`http://localhost:5000/search/tournamentsByGround?ground=${encodeURIComponent(search.trim())}`)
          .then(res => res.json())
          .then(data => {
            setMatches(Array.isArray(data) ? data : []);
          })
  ...
  useEffect(() => {
    ...
    if (selectedPlayer && email) {
      fetch(`http://localhost:5000/user/${encodeURIComponent(email)}/isFollowing/${encodeURIComponent(selectedPlayer.email)}`)
        .then(res => res.json())
  ...
  const handleFollow = () => {
    ...
    fetch(`http://localhost:5000/user/${encodeURIComponent(email)}/follow`, {
      method: 'POST',
