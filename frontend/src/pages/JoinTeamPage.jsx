...
const JoinTeamPage = () => {
  ...
  useEffect(() => {
    if (userEmail) {
      fetch(`http://192.168.125.42:5000/api/user/by-email/${userEmail}`)
        .then(res => res.json())
        .then(data => {
  ...
  const handleValidateCode = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`http://192.168.125.42:5000/api/matches/by-code/${joinCode}`);
      if (!res.ok) {
  ...
  const handleJoinTeam = async (team) => {
    ...
    setLoading(true);
    try {
      const res = await fetch('http://192.168.125.42:5000/api/matches/join-by-code', {
        method: 'POST',
